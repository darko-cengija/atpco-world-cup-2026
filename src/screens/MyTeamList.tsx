...
export default function MyTeamList() {
  ...
  return (
    <div className="px-4 py-2">
      ...
      {teams.length === 0 ? (
        <p className="text-gray-500 text-center py-10">Teams have not been added yet.</p>
      ) : (
        <Reorder.Group
          as="div"
          axis="y"
  ...
  return (
    <span
      className={`shrink-0 rounded-full border px-2.5 py-1 text-xs font-semibold ${
        locked
          ? 'border-yellow-700/60 bg-yellow-900/20 text-yellow-400'
          : status === 'error'
            ? 'border-red-800/70 bg-red-900/20 text-red-400'
            : 'border-emerald-800/70 bg-emerald-900/20 text-emerald-400'
      }`}
    >
      {label}
    </span>
...
function TeamRow({
  ...
  return (
    <Reorder.Item
      ...
      className="grid grid-cols-[2rem_2.25rem_minmax(0,1fr)_auto] items-center gap-2 rounded-lg border border-brand-border bg-brand-card px-2.5 py-2"
    >
      ...
      <span className="block min-w-0 truncate text-sm font-semibold text-white max-[360px]:text-xs">
        {localizedName}
      </span>
...
function IconButton({
  ...
  return (
    <button
      ...
      disabled={disabled}
      onClick={onClick}
      className="flex h-8 w-8 items-center justify-center rounded-md text-gray-500 transition-colors hover:bg-brand-border hover:text-white disabled:cursor-not-allowed disabled:opacity-35"
    >
      {children}
    </button>
  )
      ...
      >
        <div className="mb-3 flex items-center gap-3">
          ...
          <div className="min-w-0">
            <h2 className="truncate text-lg font-bold text-white">{localizedName}</h2>
            <p className="text-xs text-gray-500">Current #{currentIndex + 1}</p>
          </div>
        </div>
